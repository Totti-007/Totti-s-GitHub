...
- (CTCommandCollection *)commmandDescriptors
{
    ...
    //Get auto test
    command = [[CTCommandDescriptor alloc] initWithName:@"isAuto"
                                               selector:@selector(isAuto:)
                                            description:@"Get auto test"];
    [collection addCommand:command];
    // open
...
//Get auto
-(void)isAuto:(CTTestContext *)context{
    [context runTest:^CTRecordStatus (NSError *__autoreleasing *error) {
        
        context.output = _isAuto;
        return CTRecordStatusPass;
    }];
}
...
//    [self printLog:[thisUnitInfo description]];
