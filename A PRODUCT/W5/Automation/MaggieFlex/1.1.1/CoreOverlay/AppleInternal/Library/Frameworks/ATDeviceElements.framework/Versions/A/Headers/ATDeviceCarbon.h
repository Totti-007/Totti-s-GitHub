 ...
 *
 * param[in] uartID:                        UART ID 1 to 6.
 * param[in] writeLen:                      Number of bytes to write. Max 256-32 = 224.
 * param[in] readTerminator:                UART read will stop when see this terminator. Max number of character is 32. Pass nil if don't to read
 * param[out] actualRead:                   The Actual Read only valid if readTerminator is non nil. Max it's 255 bytes
 * param[inout] data:                       The Data to read and write. Size Must be at least 256 byte.
 * param[in] uartReadTimeOutInMiliSeconds:  read time out in Milio seconds
 * param[out] aError: nil = NO error, or error set. Error code is defined in <ATDeviceElements/ATDeviceCarbonErrors.h>
 * retval BOOL: YES = sucess NO = Error;
 *
 *
